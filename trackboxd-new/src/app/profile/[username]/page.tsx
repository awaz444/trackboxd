...
async function getProfileData(username: string): Promise<ProfileData | null> {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/profile/${username}`, {
    });
...
export async function generateMetadata({ params }: ProfilePageProps) {
  ...
  return {
    ...
    openGraph: {
      ...
      images: profileData.user.image_url ? [profileData.user.image_url] : [],
    },
  };
}
...
export default async function ProfilePage({ params }: ProfilePageProps) {
  ...
  const likedActivity = recentActivity?.filter(a => a?.type === "like")?.slice(0, 4) || [];
  const reviewedActivity = recentActivity?.filter(a => a?.type === "review")?.slice(0, 4) || [];
  const annotatedActivity = recentActivity?.filter(a => a?.type === "annotation")?.slice(0, 4) || [];
  ...
  return (
    <div className="min-h-screen bg-[#FFFBEb]">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {/* Recent Activity */}
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-[#5C5537] mb-6">Recent Activity</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-bold text-[#5C5537]">Recently Liked</h3>
              </div>
              <div className="space-y-3">
                {likedActivity?.map(activity => (
                  activity && <ActivityCard key={activity.id} activity={activity} />
                ))}
                {(!likedActivity || likedActivity.length === 0) && (
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-bold text-[#5C5537]">Recently Reviewed</h3>
              </div>
              <div className="space-y-3">
                {reviewedActivity?.map(activity => (
                  activity && <ActivityCard key={activity.id} activity={activity} />
                ))}
                {(!reviewedActivity || reviewedActivity.length === 0) && (
                  <div className="text-center text-[#5C5537]/70 py-8">
                    No recent reviews
                  </div>
                )}
          ...
          <div className="mt-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-bold text-[#5C5537]">Recently Annotated</h3>
            </div>
            <div className="space-y-3">
              {annotatedActivity?.map(activity => (
                activity && <ActivityCard key={activity.id} activity={activity} />
              ))}
              {(!annotatedActivity || annotatedActivity.length === 0) && (
                <div className="text-center text-[#5C5537]/70 py-8">
