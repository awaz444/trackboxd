...
const CompactAnnotationCard: React.FC<CompactAnnotationCardProps> = ({ annotation }) => {
  const { user } = useUser();
  ...
  const trackName = annotation.track_details?.name || annotation.item?.name || "Unknown Track";
  const artistNames = annotation.track_details?.artists?.map(a => a.name).join(", ") || annotation.item?.artist || "Unknown Artist";
  const albumName = annotation.track_details?.album?.name || annotation.item?.album || "";
  const coverUrl = annotation.track_details?.album?.images?.[0]?.url || annotation.item?.cover_url || "";
  ...
  useEffect(() => {
    const checkLikeStatus = async () => {
      ...
      try {
        const res = await fetch(`/api/likes/annotation?userId=${user.id}&annotationId=${annotation.id}`);
      ...
      >
        <div className="flex items-start gap-2">
          <div className="flex-1 min-w-0">
            ...
            {annotation.text && (
              <p className="text-[#5C5537] text-sm line-clamp-2 mb-2">{annotation.text}</p>
            )}
